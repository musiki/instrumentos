...
function visualizeWaveform(audioBlob) {
  ...
  waveformInstance.load([{ src: URL.createObjectURL(audioBlob), name: "Audio" }])
    .then(() => {
      // Get buffer length and resize waveform
      Tone.getContext().rawContext.decodeAudioData(audioBlob.arrayBuffer())
        .then((buffer) => {
