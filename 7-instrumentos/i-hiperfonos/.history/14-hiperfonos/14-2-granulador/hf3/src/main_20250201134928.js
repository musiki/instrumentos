...
async function visualizeWaveform(audioBlob) {
  ...
  try {
    // Ensure we get the correct ArrayBuffer before waveform loads
    const arrayBuffer = await audioBlob.arrayBuffer();
    audioBuffer = await Tone.getContext().rawContext.decodeAudioData(
      arrayBuffer
    );

    // Initialize WaveformPlaylist with EventEmitter
  ...
  waveformInstance
    ...
    .then(async () => {
      ...
      try {
        const arrayBuffer = await audioBlob.arrayBuffer(); // <-- Await Blob conversion
        audioBuffer = await Tone.getContext().rawContext.decodeAudioData(
          arrayBuffer
        );
        updateHUD();
